...
`0.4.1` 发布（2026-09-20）：入口块机制退役，规程仅由用户显式请求经全局 Skill 通道加载，
`setup.py` 只保留 `status`/`disable` 清理旧入口块。GitHub tag 为 `v0.4.1`，Actions 结果待记录。
...
`docs/verification/2026-09-14-simple-handoff.md`，10 个真实 Codex 场景全部通过。GitHub tag 为
`v0.4.0`（公开仓提交 `edfd6ff`），Actions run `34815619413` 全绿：CI 轮询窗口内尚未转公开打了一条
warning，发布后数分钟用版本 API 复核，12 个上传文件全部公开、`comm` 缺集为空。
...
**审计结论暂缺，发布后一切扫描结论在终态前都算暂定**：`0.3.0`、`0.4.0` 与 `0.4.1` 均需待 VirusTotal 与
...
| 渠道 | 状态 | 地址 | 说明 |
| --- | --- | --- | --- |
| GitHub | 已发布 | <https://github.com/SnowsonZ/agent-handoff-skill> | `v0.4.1`（2026-09-20）；`v0.4.0` run `34815619413`、`v0.3.0` run `34715721480` 成功 |
| skills.sh | 已收录 | <https://www.skills.sh/snowsonz/agent-handoff-skill/handoff-installer> | 地址随 2026-09-02 GitHub 仓库改名一并变化 |
| ClawHub | `0.4.1` 已推 tag，审计待终态 | <https://clawhub.ai/snowsonz/skills/agent-handoff-skill/security-audit> | `0.4.0` 版本 API 12/12 一致；VirusTotal、SkillSpector 回写待复查 |
