...
删除 owner、ack、hop、特殊 commit trailer、强制任务分支与提交流程，保留旧格式读取和旧账本查询。
压缩不再依赖已提交基线，未提交记录也可整理；关键约束、待办、理由与失败教训必须保留。

全局 Skill 名称仍为 `handoff-installer`。入口仅在已有任务或明确接力请求时加载规程；
旧部署清理和入口配置改为按需维护。从旧版升级需重新 enable 已启用客户端，替换原自动迁移入口。
安装、迁移脚本继续保护业务文件。
